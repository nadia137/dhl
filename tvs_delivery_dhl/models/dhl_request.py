...
class DHLProvider():
    ...
    def _set_client(self, wsdl_filename, api):
        wsdl_path = get_resource_path('tvs_delivery_dhl', 'api', wsdl_filename)
    ...
    def _set_export_declaration(self, carrier, picking, is_return=False):
        ...
        export_declaration.InvoiceNumber = carrier.env['ir.sequence'].sudo().next_by_code('tvs_delivery_dhl.commercial_invoice')
        if is_return:
